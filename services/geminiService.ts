...
export const generateSketch = async (
  ...
  // Use generateRenderPrompt to create a structured prompt even for sketch/render conversions if needed, 
  // or use a simplified one. For now, we will use a simplified logic but could benefit from the helper.
  // We'll keep the basic prompt here to avoid complexity since SketchConverter doesn't pass photography params anymore.
...
/**
 * Helper: Analyze individual asset DNA using Gemini Flash
 */
const analyzeAssetDNA = async (ai: GoogleGenAI, asset: IdeaAsset): Promise<string> => {
  if (!asset.image) return `${asset.label} tại tọa độ (${asset.x.toFixed(1)}%, ${asset.y.toFixed(1)}%)`;

  const prompt = `
    Phân tích vật thể trong ảnh này. Trả về một câu mô tả ngắn gọn về loại vật thể, màu sắc, chất liệu và phong cách.
    Ví dụ: "Một chiếc ghế Tiffany màu vàng gold, đệm trắng, phong cách sang trọng".
    CHỈ trả về văn bản mô tả.
  `;
  ...
  try {
    ...
    const description = response.text?.trim() || "Vật thể trang trí";
    return `${asset.label} (${description}) tại vị trí tọa độ (${asset.x.toFixed(1)}%, ${asset.y.toFixed(1)}%)`;
  } catch (e) {
    console.warn(`Failed to analyze asset ${asset.label}`, e);
    return `${asset.label} tại tọa độ (${asset.x.toFixed(1)}%, ${asset.y.toFixed(1)}%)`;
  }
};
...
export const generateIdeaRender = async (
  ...
  // --- BƯỚC 1: Phân tích DNA của từng vật thể (Parallel) ---
  if (onStatusUpdate) onStatusUpdate("Đang phân tích DNA các vật thể...");
  
  // Filter assets that have images for analysis
  const validAssets = assets.filter(a => a.image !== null);
  const assetDescriptions = await Promise.all(validAssets.map(asset => analyzeAssetDNA(ai, asset)));
  
  // Add descriptions for assets without images (just using label and position)
  const textOnlyAssets = assets.filter(a => a.image === null).map(a => 
      `${a.label} tại vị trí tọa độ (${a.x.toFixed(1)}%, ${a.y.toFixed(1)}%)`
  );
  
  const allAssetDescriptions = [...assetDescriptions, ...textOnlyAssets].join('\n');

  // --- BƯỚC 2: Lập Kịch bản Master Prompt (Gemini Flash) ---
  if (onStatusUpdate) onStatusUpdate("Đang xây dựng kịch bản không gian 3D...");

  const spatialMappingPrompt = `
      Tôi có một ảnh phác thảo sự kiện (đính kèm). 
      Tôi muốn bạn đóng vai trò là Kiến trúc sư diễn họa. 
      Hãy lập một Master Prompt chi tiết để render ảnh này thành 3D Photorealistic.
      
      DANH SÁCH VẬT THỂ VÀ VỊ TRÍ (Tọa độ % tính từ trái-trên):
      ${allAssetDescriptions}
      
      YÊU CẦU:
      - Đặt các vật thể chính xác vào vị trí tọa độ đã nêu trên ảnh phác thảo.
      - Giữ nguyên bố cục và góc phối cảnh của ảnh phác thảo.
      - Các vật thể phải được render hòa hợp với nhau về ánh sáng và đổ bóng trong không gian 3D.
      - Phong cách: Sang trọng, chân thực, độ phân giải 8k.
      
      Trả về duy nhất Master Prompt bằng tiếng Anh.
  `;
  ...
  try {
      const planningResponse = await ai.models.generateContent({
          ...
          contents: {
              parts: [
                  { text: spatialMappingPrompt },
                  { inlineData: { mimeType: sketchImage.mimeType, data: sketchImage.base64 } }
              ]
          }
      });
      masterPrompt = planningResponse.text || "";
  } catch (e) {
      console.error("Failed to generate master prompt", e);
      throw new Error("Không thể lập kịch bản diễn họa.");
  }

  // --- BƯỚC 3: Render Final (Gemini Pro Image) ---
  if (onStatusUpdate) onStatusUpdate("Đang Render 3D hoàn thiện...");

  // We use the sketch as the visual structure reference and the prompt for style/content
  const parts: any[] = [
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: { parts: parts },
      config: {
        systemInstruction: "You are an advanced AI 3D Visualizer. Use the provided image strictly as the STRUCTURAL REFERENCE (Structure Control). Render the scene described in the prompt, placing objects exactly as requested while maintaining the perspective of the reference image."
      }
    });
    ...
    console.error("Gemini Idea Render Error:", error);
    throw error;
  }
};